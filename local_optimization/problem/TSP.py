...
class TSP:
    ...
    def __init__(self, points: list[tuple[int]], paths: list[Path] = None, all_points=None) -> None:
    ...
    def __draw_all_points(self) -> None:
        """Draws 2D points and their coordinates on the canvas."""

        self.__ax.scatter(
            *array(self.__all_points).T,
            s=0.1,
            zorder=1,
            color=TSP.CLR_A_POINT,
            label=f"Points ({len(self.__all_points)})",
        )
